...
(async () => {
  ...
  try {
    ...
    assert.match(await page.locator('.ruler-row').first().innerText(), /Ulysses/);
    await page.locator('.ruler-evidence summary').first().click();
    assert.match(await page.locator('.ruler-evidence a').first().getAttribute('href'), /^https:\/\//);
    ...
    const report = { passed: true, checks: ['ruler selection', 'year updates preserve roster DOM', 'keyboard expansion preserves paused playback', 'evidence links', 'mobile overflow', 'sample-reviewed source label', 'unverified coverage', 'polity shortcuts removed', 'search selection', 'no page errors'], screenshots: ['desktop.png', 'mobile.png'] };
